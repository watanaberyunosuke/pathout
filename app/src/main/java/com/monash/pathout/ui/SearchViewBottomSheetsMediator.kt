...
class SearchViewBottomSheetsMediator(
    ...
    onNavigatePlaceClicked: (SearchPlace) -> Unit,
) {
            ...
            addOnSearchPlaceAddedToFavoritesListener { searchPlace, favorite ->
                ...
                if (lastTransaction != null && lastTransaction.screen == Screen.PLACE) {
                    screensStack.push(
                        Transaction(
                            Screen.PLACE,
                            searchPlace.copy(record = favorite)
                        )
                    )
                } else {
    ...
    fun onRestoreInstanceState(savedInstanceState: Bundle) {
        val savedStack =
            savedInstanceState.getParcelableArrayList<Transaction>(KEY_STATE_EXTERNAL_BACK_STACK)
                ?: return
        screensStack.clear()
    ...
    // Should work quite fast as we get last known location.
    ...
    private companion object {
        ...
        const val KEY_STATE_EXTERNAL_BACK_STACK =
            "SearchViewBottomSheetsMediator.state.external.back_stack"
